...
class VehicleService
{
    ...
    public function GetVehicle($vehicleId)
    {
        try {
            ...
            if (app()->environment('local')) {
                foreach ($this->readLocalVehicles() as $row) {
    ...
    public function GetVehicleList()
    {
        try {
            ...
            error($e, __FILE__, __METHOD__, __LINE__);
            if (app()->environment('local')) {
                return collect($this->readLocalVehicles())->map(fn ($r) => (object)$r);
            }
            return false;
        }
    }
    ...
    public function create($vehicleName)
    {
        try {
            ...
            error($e, __FILE__, __METHOD__, __LINE__);
            if (app()->environment('local')) {
                $rows = $this->readLocalVehicles();
                $maxId = 0;
                foreach ($rows as $r) $maxId = max($maxId, (int)($r['id'] ?? 0));
                $rows[] = ['id' => $maxId + 1, 'vehicle_type' => 1, 'vehicle_name' => (string)$vehicleName, 'sort_number' => count($rows) + 1];
                return $this->writeLocalVehicles($rows);
            }
            return false;
        }
    }
    ...
    public function delete($vehicleId)
    {
        try {
            ...
            error($e, __FILE__, __METHOD__, __LINE__);
            if (app()->environment('local')) {
                $rows = array_values(array_filter($this->readLocalVehicles(), fn ($r) => (int)($r['id'] ?? 0) !== (int)$vehicleId));
                return $this->writeLocalVehicles($rows);
            }
            return false;
        }
    ...
    private function readLocalVehicles(): array
    {
        $path = storage_path(self::LOCAL_VEHICLE_FILE);
    ...
    private function writeLocalVehicles(array $rows): bool
    {
